...
These variables are accessible throughout adore and can be used in any command context which contains a `message` or `embed code` parameter (e.g. `welcome`, `leave`, `boost`, `birthday`, and vanity claim notifications).
...
<Tabs>
  ...
  <Tab title="Birthday">
    <Info>
      Only available for `,birthday message` guild announcements.
    </Info>
    ...
    | `{birthday.channel}` | Configured birthday announcement channel |

    Standard [user](#user) and [guild](#guild) variables also work.
  </Tab>
