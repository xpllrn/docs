...
<CodeGroup>
    ...
    ```javascript Example
    ,voicemaster join role @in iasmine's vc
    ```
...
<Card title="Subscription Required" icon="medal" iconType="duotone" horizontal>
  You must have [Personal Premium](/overview/donator-perks) to use the
  `voicemaster hide` command.
</Card>
